RSpec.describe 'reduce' do
  ...
  it 'multiplies a list of numbers' do
    numbers = [2, 3, 5, 7]
    result = numbers.reduce do |product, number|
      product * number
    end
  ...
  it 'capitalize key words in phrase' do
    ...
    result = keywords.reduce(phrase) do |phrase,word|
  ...
  it 'subtract smallest values from 100' do
    ...
    result = elements.reduce(100) {|difference, arr| difference - arr.min}
  ...
  it 'adds all second values together' do
    ...
    # initial value is 0
    result = elements.reduce(0) {|sum,arr| sum + arr[-1]}
